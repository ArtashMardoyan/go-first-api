...
#   ./scripts/deploy.sh
#
# Steps: build (linux/amd64) -> push to ECR (:latest + git SHA) ->
# App Runner auto-redeploys -> sync the Postman collection.
#
# Config can be overridden via env vars (defaults match docs/001-deploy-aws-apprunner-rds.md).
...
ECR_REPO="${ECR_REPO:-go-first-api}"
SERVICE_ARN="${SERVICE_ARN:-arn:aws:apprunner:us-east-2:813021164486:service/go-first-api/b960c0aea4474e7e91fa763fe0fef30e}"
SERVICE_URL="${SERVICE_URL:-https://6csnsif6gd.us-east-2.awsapprunner.com}"
...
# 5. sync the Postman collection (non-fatal — a sync hiccup shouldn't fail the deploy)
echo "==> Syncing Postman collection"
if ! ./scripts/sync-postman.sh; then
  echo "warning: Postman sync failed (deploy itself succeeded)" >&2
fi
