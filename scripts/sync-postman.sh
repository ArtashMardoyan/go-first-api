...
#
# Usage:
#   POSTMAN_API_KEY=PMAK-xxxx ./scripts/sync-postman.sh
#
# The collection UID can be overridden via POSTMAN_COLLECTION_UID.
# NEVER hardcode the API key here — pass it via the environment.

set -euo pipefail

COLLECTION_FILE="${COLLECTION_FILE:-postman/go-first-api.postman_collection.json}"
COLLECTION_UID="${POSTMAN_COLLECTION_UID:-5792649-3472e6e0-3219-4f5c-98ec-feb019b350f4}"
...
if [[ -z "${POSTMAN_API_KEY:-}" ]]; then
  echo "error: POSTMAN_API_KEY is not set" >&2
  echo "  export POSTMAN_API_KEY=PMAK-... and re-run" >&2
  exit 1
fi
...
echo "Syncing $COLLECTION_FILE -> Postman collection $COLLECTION_UID ..."
